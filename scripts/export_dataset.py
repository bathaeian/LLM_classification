...
def export_dataset(
    ...
    if num_classes > len(class_dirs):
        raise ValueError(f"Dataset only contains {len(class_dirs)} classes.")
    ...
    for class_dir in selected_classes:
        ...
        if samples_per_class > len(json_files):
            raise ValueError(f"Class {label} only has {len(json_files)} samples.")

        chosen = rng.sample(json_files, samples_per_class)

        for json_file in chosen:
